...
def main():
    ...
    image_1_path = "../datasets/MSCOCO/000000055857.jpg"
    image_2_path = "../datasets/MSCOCO/000000574769.jpg"


    ### Step 1: Set the max number of tiles in `max_num`
    pixel_values_1 = load_image(image_1_path, max_num=12).to(torch.bfloat16).cuda()
    ...
    response = model.chat(tokenizer, pixel_values_1, question, generation_config)
    print(f'User: {question}\nAssistant: {response}')
    input()
    ...
    response, history = model.chat(tokenizer, pixel_values_1, question, generation_config, history=None, return_history=True)
    print(f'User: {question}\nAssistant: {response}')
    question = 'Please write a poem according to the image.'
    response, history = model.chat(tokenizer, pixel_values_1, question, generation_config, history=history, return_history=True)
    print(f'User: {question}\nAssistant: {response}')
    input()
    ...
    pixel_values1 = load_image(image_1_path, max_num=12).to(torch.bfloat16).cuda()
    pixel_values2 = load_image(image_2_path, max_num=12).to(torch.bfloat16).cuda()
    ...
    ### Step 7: batch inference, single image per sample (单图批处理)
    pixel_values1 = load_image(image_1_path, max_num=12).to(torch.bfloat16).cuda()
    pixel_values2 = load_image(image_2_path, max_num=12).to(torch.bfloat16).cuda()
    num_patches_list = [pixel_values1.size(0), pixel_values2.size(0)]
    pixel_values = torch.cat((pixel_values1, pixel_values2), dim=0)
